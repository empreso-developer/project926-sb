...
public final class TicketConfirmationEmailTemplate {
  ...
  public static Rendered render(TicketConfirmationEmailData data, String qrContentId) {
    ...
    String firstName = (customer.firstName() != null && !customer.firstName().isBlank()) ? customer.firstName()
        : "there";
    ...
    for (var t : data.tickets()) {
      ticketRows.append(
          """
              ...
              .formatted(escapeHtml(t.name()), t.quantity(),
                  escapeHtml(EmailFormatUtils.formatCurrency(t.subtotal(), payment.currency()))));
    }
