...
/**
 * Mirrors app/(project926)/project926/dashboard/organizer/events/[id]/attendees/page.tsx
...
public class AttendeeService {
    ...
    public AttendeeListResponse listAttendees(UUID eventId, String callerId, Integer pageParam, String filterParam, String searchParam) {
        ...
        List<AttendeeDto> attendees = bookings.stream()
            .map(b -> toAttendeeDto(b, profilesById.get(b.getCustomerId()), ticketsByBooking.getOrDefault(b.getId(), List.of())))
            .toList();
    ...
    private AttendeeStatsDto computeStats(UUID eventId) {
        ...
        List<BookingItem> items = confirmedBookingIds.isEmpty() ? List.of() : bookingItemRepository.findByBookingIdIn(confirmedBookingIds);
    ...
    private Map<UUID, List<TicketCountDto>> groupTicketCountsByBooking(List<UUID> bookingIds) {
        ...
        return items.stream().collect(Collectors.groupingBy(
            ...
            Collectors.mapping(item -> new TicketCountDto(
                    ticketTypesById.containsKey(item.getTicketTypeId()) ? ticketTypesById.get(item.getTicketTypeId()).getName() : "Ticket",
                    item.getQuantity()
                ),
                Collectors.toList())
        ));
