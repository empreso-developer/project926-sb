...
/**
 * Mirrors app/(project926)/project926/api/organizer/events/[eventId]/check-in/route.ts
...
public class CheckInService {
    ...
    public CheckInResponse checkIn(UUID eventId, String callerId, UUID bookingId, String reference) {
        ...
        if (!"confirmed".equals(booking.getStatus())) {
            return CheckInResponse.status("pending".equals(booking.getStatus()) ? "payment_not_confirmed" : "cancelled");
        }
    ...
    private CheckInResponse.AttendeeInfo loadAttendeeInfo(String customerId, UUID bookingId) {
        ...
        List<TicketCountDto> ticketTypes = items.stream()
            .map(i -> new TicketCountDto(
                ticketTypesById.containsKey(i.getTicketTypeId()) ? ticketTypesById.get(i.getTicketTypeId()).getName() : "Ticket",
                i.getQuantity()
            ))
            .toList();
